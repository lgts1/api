...
export default {
   ...
   async createuser(user_code: string, tokenuser: string, atkuser: string, balance: number, agentid: number) {
      const res = await promisePool.query<ResultSetHeader>("INSERT INTO users (username,token,atk,saldo,agentid,isinfluencer) VALUES(?,?,?,?,?,1)", [user_code, tokenuser, atkuser, balance, agentid])
      return res[0]
   },
